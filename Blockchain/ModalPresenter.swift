...
@objc class ModalPresenter: NSObject {
    ...
    private var rootView: UIView? {
        return AppCoordinator.shared.window.rootViewController?.view
    }
    ...
    @objc func showModal(withContent content: BCModalContentView, closeType: ModalCloseType, showHeader: Bool, headerText: String, onDismiss: OnModalDismissed? = nil, onResume: OnModalResumed? = nil) {
        ...
        content.prepareForModalPresentation()
        
        modalViewToShow.myHolderView.addSubview(content)
        content.frame = CGRect(x: 0, y: 0, width: modalViewToShow.myHolderView.frame.size.width, height: modalViewToShow.myHolderView.frame.size.height)
